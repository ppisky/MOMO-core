...
MOMO Core 1.0.0-rc.3 已完整实现可选的实验性 `momo.ddm/1` 投影契约。这里的
...
| 计算 | 确定性的 logit 加法和乘法 profile、有界效果、缺失输入中性 |
| 治理输入 | 封闭且带类型的 DMW、NSG、MO State、scene 和 request 信号族 |
| 选择 | 作者硬约束、确定性互斥组、top-k，以及 latent/salient/dominant 表达区间 |
| 迟滞 | 前一轮区间按 `(managed_space_id, conversation_id, character_id)` 持久化；profile revision 变化时重置 |
| 原子性 | 检索正文、DMW/NSG/scene 指纹和观测在同一个有序来源锁窗口内取得；下一轮 DDM 区间与 MO State snapshot 在同一 SQLite 事务发布 |
...
Rust 测试覆盖 profile 解析与拒绝、中性输入、边界、正负调制单调性、确定性重放、来源
指纹、scene/request 类型、互斥冲突、top-k、持久化迟滞、跨 scope 隔离，以及 snapshot
与下一轮区间的原子发布。服务端契约测试还验证了 profile 管理、表达提示实际进入对话
