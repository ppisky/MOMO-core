...
>
> **2026-09-10 更新：** 同一 `qwen3.8-flash` 的三题 12 轮诊断先复现了实体转写、
> 治理输出截断、回复冗长和无证据新增条件。Core 随后加入正常结束校验、一次有界修复、
> 多语言回归覆盖，并把 Roleplay Director 移到资产证据之后。三次改进后实跑中，
> MOMO 候选由首轮约 **66.7/100** 提升到约 **75–83.3/100**；12 轮时仍不节省 probe
> input token，也没有证据表明稳定胜过完整上下文。该小样本只用于回归定位。
...
| MO State v2 | 7.4 | 已有按 Space 锁、来源指纹、revision、Operation Journal、Snapshot、Scene 解析、恢复性维护和可选 DDM 投影，是可信的 baseline。检索正文、Scene 与来源指纹现在共享同一个有序锁窗口，DDM 迟滞状态与 snapshot 原子发布。跨 DMW/NSG 的统一 Saga、真正有界 Reconciler、软超时执行和持久 `run_id` 暂停/恢复仍未完成。 |
...
| 配置治理与能力发现 | 8.1 | allow/ignore/reject、参数白名单、能力上限、视觉回退和固定产品提示词资产边界清楚。本次删除了官方示例中 Core 实际不执行的旧宿主字段，并明确“可移植保留不等于执行”。 |
...
后续进展（2026-09-09）：多 Space MO State 已为每个读取来源持久化独立的
DMW/NSG/Scene 指纹与单调 revision，并把冻结的来源版本写入 Operation 和 Snapshot；
幂等重放继续使用原始来源版本。下面原列出的对应 P1 已完成。
