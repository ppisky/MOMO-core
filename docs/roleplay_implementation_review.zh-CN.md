...
> **2026-09-08 更新：** 本文的时间点评分没有重新计算。此后 Core 已在真实回复链路加入
> 默认启用且可审计的角色扮演导演；MORP 1.0 也已用纯角色扮演的中英双语反事实评测替代
> 混合记忆套件。下文旧缺口与测试数量应按历史证据理解。
>
> **2026-09-09 更新：** MORP 1.0 的 64 题全量实跑得分为 **88.87/100**，其中三次
> 网关终止错误按零分保留。关系动态最高，叙事连贯和世界具身最低。该结果仍是单次公开
> 合成集 provisional 成绩，不等于人类校准。
