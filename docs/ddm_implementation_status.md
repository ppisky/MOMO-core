...
MOMO Core 1.0.0-rc.3 contains a complete opt-in implementation of the
...
| Evaluation | Deterministic logit-additive and multiplicative profiles with bounded effects and neutral missing input |
| Governed inputs | Closed, typed DMW, NSG, MO State, scene, and request signal families |
| Selection | Authored hard constraints, deterministic exclusive groups, top-k selection, and latent/salient/dominant expression bands |
| Hysteresis | Previous bands persisted per `(managed_space_id, conversation_id, character_id)` and reset on profile revision change |
| Atomicity | Retrieved bodies, DMW/NSG/scene fingerprints, and observations are captured under one ordered source-lock window; the next DDM bands publish in the same SQLite transaction as the MO State snapshot |
...
Rust tests cover profile parsing and rejection, neutral input, bounds,
...
persisted hysteresis, cross-scope isolation, and atomic snapshot/band
publication. The server contract test additionally verifies profile management,
