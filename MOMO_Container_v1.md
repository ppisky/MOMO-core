...
本文档只确定 MOMO 配置与数据导出的总体方向，不在当前阶段冻结具体字段、文件数量或内部目录。MOMO 的运行配置使用 TOML；`.moc`（MOMO Container）是基于 tar.zst 的通用可移植容器，可承载用户选择导出或同步的各类 MOMO 数据。相同容器原则同时用于本地导入导出与官方云存储，避免为不同传输渠道维护彼此不兼容的数据格式。
