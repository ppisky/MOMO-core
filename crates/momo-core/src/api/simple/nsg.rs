...
pub async fn list_nsg_embedding_documents_json(owner_id: String) -> Result<String, String> {
...
pub async fn nsg_vector_status_json(
    ...
    let status = core()?
        .vector_store()
        .nsg_vector_status(owner_id, &vector_space_id, &hashes)
        .await
        .map_err(|error| error.to_string())?;
...
pub async fn run_memory_maintenance_json(owner_id: String) -> Result<String, String> {
    let owner_id = uuid::Uuid::parse_str(&owner_id).map_err(|error| error.to_string())?;
    let report = core()?
        .memory_for(owner_id)
        .map_err(|error| error.to_string())?
...
pub async fn list_nsg_pending_candidates_json(owner_id: String) -> Result<String, String> {
    let owner_id = uuid::Uuid::parse_str(&owner_id).map_err(|error| error.to_string())?;
    let memory = core()?
        .memory_for(owner_id)
        .map_err(|error| error.to_string())?;
...
pub async fn delete_nsg_node_json(owner_id: String, target_file: String) -> Result<String, String> {
    let owner_id = uuid::Uuid::parse_str(&owner_id).map_err(|error| error.to_string())?;
    let memory = core()?
        .memory_for(owner_id)
        .map_err(|error| error.to_string())?;
