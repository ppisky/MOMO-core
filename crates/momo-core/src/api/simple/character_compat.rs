...
struct ImportExternalCharacterRequest {
    scope_id: String,
    input_path: String,
    format: crate::ExternalCharacterImportFormat,
}
...
pub async fn import_external_character_json(request_json: String) -> Result<String, String> {
    ...
    let imported =
        crate::import_external_character(core()?, scope_id, request.input_path, request.format)
            .await
...
pub async fn export_preserved_character_source_json(
    ...
    let request: ExportPreservedCharacterSourceRequest =
        serde_json::from_str(&request_json).map_err(|error| error.to_string())?;
    let scope_id = uuid::Uuid::parse_str(&request.scope_id).map_err(|error| error.to_string())?;
    let character_id =
        uuid::Uuid::parse_str(&request.character_id).map_err(|error| error.to_string())?;
