...
# MORP's primary score is role-play quality. The former memory benchmark
# dimensions remain readable for archived datasets, but are not generated by
# the default suite and never enter a role-play report.
ROLEPLAY_DIMENSIONS = (
    "character_fidelity",
...
LEGACY_DIMENSIONS = ("recall", "update", "reasoning", "boundary", "world", "persona", "emotion", "social")
DIMENSIONS = ROLEPLAY_DIMENSIONS + LEGACY_DIMENSIONS
...
def candidate_case(case):
    ...
    public = {"case_id": case["id"], "persona": case["persona"], "query": case["query"],
              "history": [{k: event[k] for k in ("id", "session", "time", "role", "text") if k in event}
    ...
    if case.get("evaluation_mode") == "roleplay":
        public["evaluation_mode"] = "roleplay"
    return public
